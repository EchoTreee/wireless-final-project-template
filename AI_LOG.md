...
## 阶段 6 — 根据老师课堂讲评的二次改进（feedback → 修订闭环）

**背景**：老师对全班 20 份 PR 做了 80 道隐藏测试并课堂讲评，指出最大共性问题是**参数校验**——`invalid_snr` 涉及 **20/20 全部提交**、`invalid_modulation` 涉及 5 份，即 CLI 未明确拒绝非法输入。老师给了再次提交的机会。

**自查（实测对照）**：逐条对照老师公布的问题清单实测本项目 PR #16——UTF-8 长度、padding、hidden_texts、anti_hardcode、产物完整性、图像解释、metrics 字段均已达标；**唯一缺口是参数校验**：实测 `--mod foobar` / `--channel foobar` / `--snr -100` 均静默接受、退出码 0。

**修改**：给 `main.py` 的 `--mod`/`--channel` 加白名单 `choices`，`--snr` 加 [-50, 100] dB 有限数校验（自定义 `_snr_arg`）；非法输入明确报错并非 0 退出。新增 `tests/test_cli_validation.py`（5 项）。

**验证**：非法参数现均 `rc=2` + 明确错误信息；合法参数 `rc=0` 不受影响；单元测试 52 项（+5）、公开测试 22 项全绿，无回归。DESIGN 更新至 v0.3。

**采纳理由**：这是老师明示的全班重灾区，投入小、正中要害；也是又一个 feedback → 修订闭环——把外部评审反馈沉淀为规格约束（DESIGN v0.3）与回归测试（test_cli_validation）。
